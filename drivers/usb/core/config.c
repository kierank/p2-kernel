/* $Id: config.c 9139 2010-09-14 00:43:11Z Noguchi Isao $ */
...
#else /* 2010/7/9 modified by Panasonic for little-endian access to
         the descriptors */
	if (usb_endpoint_xfer_isoc(&ep->desc)) {
		max_tx = __le16_to_cpu(ep->desc.wMaxPacketSize) * (desc->bMaxBurst + 1) *
			(desc->bmAttributes + 1);
	} else if (usb_endpoint_xfer_int(&ep->desc)) {
		max_tx = __le16_to_cpu(ep->desc.wMaxPacketSize) * (desc->bMaxBurst + 1);
	} else {
...
#else /* 2010/7/9 modified by Panasonic for little-endian access to
         the descriptors */
	if (__le16_to_cpu(desc->wBytesPerInterval) > max_tx) {
		dev_warn(ddev, "%s endpoint with wBytesPerInterval of %d in "
				"config %d interface %d altsetting %d ep %d: "
				"setting to %d\n",
				usb_endpoint_xfer_isoc(&ep->desc) ? "Isoc" : "Int",
                 __le16_to_cpu(desc->wBytesPerInterval),
				cfgno, inum, asnum, ep->desc.bEndpointAddress,
				max_tx);
		desc->wBytesPerInterval = __cpu_to_le16(max_tx);
	}
#endif
valid:
	return retval;
