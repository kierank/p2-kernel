 ...
 */
/* $Id: xhci-ext-caps.h 9139 2010-09-14 00:43:11Z Noguchi Isao $ */

#ifndef __XHCI_EXT_CAPS_H__
#define __XHCI_EXT_CAPS_H__
...
/* xHCI Supported Protocol Capability  - section 7.2 */
struct xhci_ext_cap_protocol_info {
    u8  rev_major;
    u8  rev_minor;
    u8  port_offset;
    u8  port_count;
...
/* offset 00h */
#define XHCI_EXT_CAPS_REV_MAJOR(p) (((p)>>24)&0xf)
#define XHCI_EXT_CAPS_REV_MINOR(p) (((p)>>16)&0xf)
/* offset 08h */
#define XHCI_EXT_CAPS_PORT_OFFSET(p) (((p)>>0)&0xf)
#define XHCI_EXT_CAPS_PORT_COUNT(p) (((p)>>8)&0xf)
/* <--- 2010/8/3, added by Panasonic */
...
static inline int xhci_find_next_cap_offset(void __iomem *base, int ext_offset)
{
	...
	if (ext_offset == XHCI_HCC_PARAMS_OFFSET){
        ...
        ext_offset = 0;         /* 2010/6/28, fixed bug by Panasonic */
	}else
...
/* 2010/8/3, added by Panasonic ---> */
/*
 *  get infomation of xHCI Supported Protocol Capability
 *
 * @base PCI MMIO registers base address.
 * @ext_offset Offset from base of the first extended capability to look at,
 * 		or the address of HCCPARAMS.
 * @info    return structure
 */
static inline int xhci_get_ext_cap_protocol_info(void __iomem *base, int ext_offset,
                                                     struct xhci_ext_cap_protocol_info *info)
{
    ...
    new_ext_offset= xhci_find_ext_cap_by_id(base, ext_offset, XHCI_EXT_CAPS_PROTOCOL);
    if (!new_ext_offset)
        return 0;

    val_00 = readl(base + new_ext_offset + 0x00);
    info->rev_major = XHCI_EXT_CAPS_REV_MAJOR(val_00);
    info->rev_minor = XHCI_EXT_CAPS_REV_MINOR(val_00);

    val_08 = readl(base + new_ext_offset + 0x08);
    info->port_offset = XHCI_EXT_CAPS_PORT_OFFSET(val_08);
    info->port_count = XHCI_EXT_CAPS_PORT_COUNT(val_08);

    return new_ext_offset;
}
/* <--- 2010/8/3, added by Panasonic */

#endif  /* __XHCI_EXT_CAPS_H__ */
