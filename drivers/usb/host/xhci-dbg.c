 ...
 */
/* $Id: xhci-dbg.c 9139 2010-09-14 00:43:11Z Noguchi Isao $ */
...
void xhci_print_trb_offsets(struct xhci_hcd *xhci, union xhci_trb *trb)
{
	...
	for (i = 0; i < 4; ++i)
		xhci_dbg(xhci, "Offset 0x%x = 0x%x\n",
                 i*4, xhci_desc_readl(xhci, &trb->generic.field[i]));
#endif
}
...
void xhci_dbg_erst(struct xhci_hcd *xhci, struct xhci_erst *erst)
{
	...
	for (i = 0; i < erst->num_entries; ++i) {
		...
		xhci_dbg(xhci, "@%08x %08x %08x %08x %08x\n",
				(unsigned int) addr,
                 lower_32_bits(xhci_desc_read_64(xhci, &entry->seg_addr)),
                 upper_32_bits(xhci_desc_read_64(xhci, &entry->seg_addr)),
                 (unsigned int) xhci_desc_readl(xhci, &entry->seg_size),
                 (unsigned int) xhci_desc_readl(xhci, &entry->rsvd));
#endif
		addr += sizeof(*entry);
	}
}
