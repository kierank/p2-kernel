 ...
 */
/* $Id: fsl_usb2_udc_3d.c 27465 2013-01-28 11:41:54Z Horita Seiji $ */
...
/* 2011/4/8, added by Panasonic (PAVBU) --> */
            if (!(tmp_epctrl & EPCTRL_TX_EP_STALL))
                return;
/* <-- 2011/4/8, added by Panasonic (PAVBU) */
			tmp_epctrl &= ~EPCTRL_TX_EP_STALL;
			tmp_epctrl |= EPCTRL_TX_DATA_TOGGLE_RST;
...
/* 2011/4/8, added by Panasonic (PAVBU) --> */
            if (!(tmp_epctrl & EPCTRL_RX_EP_STALL))
                return;
/* <-- 2011/4/8, added by Panasonic (PAVBU) */
			tmp_epctrl &= ~EPCTRL_RX_EP_STALL;
			tmp_epctrl |= EPCTRL_RX_DATA_TOGGLE_RST;
...
/* 2011/4/8, added by Panasonic (PAVBU) --> */
/* reset queue for an I/O request to an endpoint */
static int fsl_reset_queue(struct usb_ep *ep)
{
    if(!ep)
        return -EINVAL;

    nuke(container_of(ep, struct fsl_ep, ep), -ESHUTDOWN);
...
static void reset_irq(struct fsl_udc *udc)
{
	...
	if (fsl_readl(&dr_regs->portsc1) & PORTSCX_PORT_RESET) {
        //		VDBG("Bus reset");
		dev_info(&udc->gadget.dev, "*** Bus reset\n");
		/* Bus is reseting */
		udc->bus_reset = 1;
		...
		dev_info(&udc->gadget.dev, "*** Controller reset\n");
