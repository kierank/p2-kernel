 ...
 */
/* $Id: p2_mass_storage_3d.c 27465 2013-01-28 11:41:54Z Horita Seiji $ */
...
/* 			P2DEBUG("BulkOnly MassStorageReset (%d)\n", CurrentState); */
            dev_info(&dev->gadget->dev,"*** BulkOnly MassStorageReset (%d)\n", CurrentState);
/* <-- 2011/4/8, added by Panasonic (PAVBU) */
...
/* 2011/4/8, added by Panasonic (PAVBU) --> */
                /* reset queue for ep1-in/out */
                usb_reset_queue(dev->in_ep);
                usb_reset_queue(dev->out_ep);
/* <-- 2011/4/8, added by Panasonic (PAVBU) */
...
/* 2011/3/15, 201111/16: modified by Panasonic (PAV) --> */
	//CurrentState = POW_ON;
    if(CurrentState>INIT) {
        P2ERROR("%s: current state is NOT 'POW_ON' nor 'INIT', %d\n",__FUNCTION__,CurrentState); 
        return -EINPROGRESS;
    }
/* <-- 2011/3/15, 201111/16: modified by Panasonic (PAV) */
	return 0;
...
/* 2011/3/15, 2011/11/16: modified by Panasonic (PAV) --> */
			if (CurrentState == POW_ON || CurrentState == INIT) {
                /* clear all queue  */
                clear_queue_info();
                CurrentState = INIT;
            }
...
/* Then we call usb_gadget_register_driver() from init() function		*/ 
	...
	case P2USB_SET_SERIAL:		/* Support High-end shoulder & mobiles	*/
		...
		if (CurrentState == POW_ON || CurrentState==INIT) { /* 2011/3/15, modified by Panasonic (SAV) */
			int len_serial;
            ...
            /* 2011/3/15, modified by Panasonic (SAV) */
			P2ERROR("Set Serial command failed due to current state is invalid (%d)\n", CurrentState);
			retVal = -EACCES;
			goto P2USB_SET_SERIAL_ERROR;
		}
...
P2USB_SET_SERIAL_ERROR:
	...
	case P2USB_SET_PRODUCT_ID:	/* Support High-end shoulder & mobiles  */
		...
		if (CurrentState == POW_ON || CurrentState==INIT) { /* 2011/3/15, modified by Panasonic (SAV) */
            ...
            /* 2011/3/15, modified by Panasonic (SAV) */
			P2ERROR("Set Product ID failed due to current state is invalid (%d)\n", CurrentState);
			retVal = -EACCES;
