/*
 * Description: Handle mapping of the flash on K286(AJ-HPD2500) board
 *
 ...
 *
 */
/* $Id: p2pf-K286.c 3558 2009-07-27 05:16:40Z Noguchi Isao $ */
 ...
 *17 : 0xF578 0000 - 0xF57F FFFF : SDI cfg1     (1.8MB)
 *18 : 0xF590 0000 - 0xF5A7 FFFF : (SDI cfg2)   (1.8MB)
 *19 : 0xF5A8 0000 - 0xF5DF FFFF : R/W filesystem area(JFFS2)  (3.5MB)
 *20 : 0xF5E0 0000 - 0xF5FF FFFF : (reserved)   (3.0MB)
 */
...
struct map_info K286_map1 = {
	.name		= "K286(AJ-HPD2500) Flash 1 Map Info",
	.size		= WINDOW_SIZE1,
	.phys		= WINDOW_ADDR1,
	.bankwidth	= 4,
};

struct map_info K286_map2 = {
	.name		= "K286(AJ-HPD2500) Flash 2 Map Info",
	.size		= WINDOW_SIZE2,
	.phys		= WINDOW_ADDR2,
	.bankwidth	= 2,
};

int __init init_K286_mtd(void)
{

  /* flash1 */
	printk(KERN_NOTICE"K286(AJ-HPD2500) flash1 device: %x at %x Partition number %d\n",
			WINDOW_SIZE1, WINDOW_ADDR1, PARTITION_NUM1);
	K286_map1.virt = ioremap(WINDOW_ADDR1, WINDOW_SIZE1);
	if (!K286_map1.virt) {
		printk("Failed to Flash1 ioremap\n");
		return -EIO;
	}

	simple_map_init(&K286_map1);
	
	mymtd1 = do_map_probe("cfi_probe", &K286_map1);
	if (mymtd1) {
		mymtd1->owner = THIS_MODULE;
                add_mtd_partitions(mymtd1, partition_info1, PARTITION_NUM1);
		printk(KERN_NOTICE "K286(AJ-HPD2500) flash1 device (%s) initialized\n",mymtd1->name);
	} else {
	  iounmap((void *)K286_map1.virt);
	  return -ENXIO;
	}

	/* flash2 */
	printk(KERN_NOTICE"K286(AJ-HPD2500) flash2 device: %x at %x Partition number %d\n",
			WINDOW_SIZE2, WINDOW_ADDR2, PARTITION_NUM2);

	K286_map2.virt = ioremap(WINDOW_ADDR2, WINDOW_SIZE2);
	if (!K286_map2.virt) {
		printk("Failed to Flash2 ioremap\n");
		return -EIO;
	}
	simple_map_init(&K286_map2);

	mymtd2 = do_map_probe("cfi_probe", &K286_map2);
	if (mymtd2) {
		mymtd2->owner = THIS_MODULE;
                add_mtd_partitions(mymtd2, partition_info2, PARTITION_NUM2);
		printk(KERN_NOTICE "K286(AJ-HPD2500) flash2 device (%s) initialized\n",mymtd2->name);
	} else {
	  iounmap((void *)K286_map2.virt);
	  return -ENXIO;
	}
...
static void __exit cleanup_K286_mtd(void)
{
  /* flash1 */
	if (mymtd1) {
...
module_init(init_K286_mtd);
module_exit(cleanup_K286_mtd);

MODULE_AUTHOR("Li Yawei");
MODULE_DESCRIPTION("MTD map driver for K286(AJ-HPD2500) board");
MODULE_LICENSE("GPL");
