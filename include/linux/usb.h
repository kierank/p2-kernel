/* $Id: usb.h 12081 2011-01-31 06:38:59Z Noguchi Isao $ */
...
/* <--- Backporting from 2.6.33 kernel. 2010/10/06, modified by Panasonic (SAV) */
    ...
    unsigned int max_sectors; /* 2010/10/06, modified by Panasonic (SAV) */
...
struct usb_device {
	...
	u8 portnum;
	u8 vportnum;       /* 2010/10/18, added by Panasonic (SAV) */
	u8 level;
...
struct usb_routing_restricion {
    int met; /* if met is not zero then the device is ignored when
                routing condition is met. */
    unsigned int busnum;
    unsigned int rootport, ss_rootport;
    u32 route;
};
